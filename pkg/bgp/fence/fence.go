...
// pointed to by m.
func (m *Meta) FromSlimObjectMeta(om *slim_metav1.ObjectMeta) {
	rev, err := strconv.ParseUint(om.ResourceVersion, 10, 64)
	if err != nil {
		// panic justification: our stale event avoidance (correctness) logic depends
		// on this, if ObjectMeta changes under us, crash immediately
		// and fix this logic.
		panic("ObjectMeta.ResourceVersion must be parsible to Uint64")
	}
	(*m).Rev = rev
...
// pointed to by m.
func (m *Meta) FromObjectMeta(om *v1.ObjectMeta) {
	rev, err := strconv.ParseUint(om.ResourceVersion, 10, 64)
	if err != nil {
		// panic justification: our stale event avoidance (correctness) logic depends
		// on this, if ObjectMeta changes under us, crash immediately
